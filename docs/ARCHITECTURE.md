...
**Architecture: "4+1" Explained**
- **4 pillarLayer networks**: Generate control points p0, p1, p2, p3
- **+1 feature stream**: `img_seq` (after standard FFN) serves as parameter `t`
- **Not 5 separate pillarLayers** - FFN is a standard transformer component

```python
# 4 pillarLayer networks for control points (each: d_model → d_model → d_model)
...
# Standard FFN (not a 5th pillar)
ffn = nn.Linear(d_model, d_model)

# Forward: sigmoid gate + pillar generation + Bezier
g = torch.sigmoid(img_seq)
img_seq = img_seq + ffn(norm(img_seq))  # img_seq becomes 't' parameter
...
**Memory cost (activation memory during forward pass):**
- Peak: 10× (B×T×D) elements = 5 tensors [B,T,D] + concat [B,T,5D]
  - Individual tensors: img_seq + 4 pillar outputs = 5×
  - Concatenated tensor: [B,T,5D] = 5×
  - Total: 5 + 5 = 10× base tensor size
- Example @ B=2, T=256, D=128, float32: 10 × 0.25 MB = 2.50 MB per block
...
| Approach | Activation Params | Auxiliary Params | Total | Activation Memory Peak |
|----------|------------------|------------------|-------|------------------------|
| ReLU baseline | 0 | 32,896 (2 layers) | 32,896 | 1× (B×T×D) |
| Input-Based | 0 | 82,560 (Linear 128→640) | 82,560 | 5× (B×T×D) |
| TrainableBezier | 512 (4×128) | 32,896 (Linear) | 33,408 | 1× (B×T×D) |
| Pillar-Based | 0 | 198,144 (4 pillars) | 198,144 | 10× (B×T×D) |

**Memory terminology:**
- **Model size (parameters)**: Persistent GPU memory for weights (column "Total")
- **Process size (activation memory)**: Temporary memory during forward pass (column "Activation Memory Peak")
- Example @ B=2, T=256, D=128, float32:
  - Base tensor [B,T,D] = 0.25 MB
  - Pillar-Based: 0.76 MB params (persistent) + 2.50 MB activations (peak) = 3.26 MB total
