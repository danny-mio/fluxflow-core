...
def main():
        ...
        if isinstance(seq_block, nn.Sequential):
            for sub_layer in seq_block:
                if isinstance(sub_layer, nn.Linear):
                    ...
                    print(f"    Layer {layer_idx}: Linear({in_f}, {out_f}, bias={bias}) = " +
    ...
    params_relu = (sum(p.numel() for p in layer1.parameters()) + 
    ...
    print("-" * 70)
    print(f"{'Input-Based (Linear 256→640)':<30} {f'{params_linear:,}':<20} " +
          "0 activation params")
    print(f"{'TrainableBezier (D=128)':<30} {f'{params_tb:,}':<20} " +
          "4×D learnable params")
    print(f"{'Pillar-Based (1 pillar, D=128)':<30} {f'{params_pillar:,}':<20} " +
          "3 layers per pillar")
    print(f"{'Pillar-Based (4 pillars)':<30} {f'{4 * params_pillar:,}':<20} " +
          "For FluxTransformerBlock")
    print(f"{'ReLU Baseline (2 layers)':<30} {f'{params_relu:,}':<20} " +
    ...
    print(f"  TrainableBezier overhead: {params_tb / params_linear2 * 100:.2f}% " +
          f"of Linear layer")
    print(f"  Pillar-Based (4 pillars): {(4 * params_pillar) / params_relu:.2f}× " +
          "ReLU (2 layers)")
