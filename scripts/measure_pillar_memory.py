...
def measure_actual_memory():
    ...
    print(
        f"  Concatenated tensor [B={B}, T_img={T_img}, D={5*D}]: {theoretical_mem['concatenated_5D']:.2f} MB"
    )
    print()
    print(f"Memory peaks:")
    print(
        f"  Before concatenation (img_seq + 4 pillars): {theoretical_mem['peak_before_concat']:.2f} MB"
    )
    ...
    print(
        f"  - Before concat: img_seq + p0 + p1 + p2 + p3 = 5 × [B,T,D] = {theoretical_mem['peak_before_concat']:.2f} MB"
    )
    print(
        f"  - During concat: 5 × [B,T,D] + 1 × [B,T,5D] = {theoretical_mem['peak_during_concat']:.2f} MB"
    )
    print()
